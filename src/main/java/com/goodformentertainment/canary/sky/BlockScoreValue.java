...
//
// public BlockScoreValue(final SkylandsConfig config) {
...
// }
//
// private Map<BlockType, Integer> configPlaceValues(final SkylandsConfig config) {
// final Map<BlockType, Integer> values = new HashMap<BlockType, Integer>();
//
// SkylandsPlugin.LOG.debug("XIS block place values:");
...
// }
// }
// SkylandsPlugin.LOG.debug(sb.toString());
//
// return values;
// }
//
// private Map<BlockType, Integer> configRemoveValues(final SkylandsConfig config) {
// final Map<BlockType, Integer> values = new HashMap<BlockType, Integer>();
//
// SkylandsPlugin.LOG.debug("XIS block remove ignores:");
// final StringBuffer sb = new StringBuffer("\n");
...
// valueType = TYPE_TREE;
// } else if (type == BlockType.Dirt || type == BlockType.CoarseDirt
// || type == BlockType.Podzol || type == BlockType.Grass || type == BlockType.Mycelium
...
// || type == BlockType.BrownWool || type == BlockType.CyanWool
// || type == BlockType.GrayWool || type == BlockType.GreenWool
// || type == BlockType.LightBlueWool || type == BlockType.LightGrayWool
// || type == BlockType.LimeWool || type == BlockType.MagentaWool
// || type == BlockType.OrangeWool || type == BlockType.PinkWool
// || type == BlockType.PurpleWool || type == BlockType.RedWool
...
// valueType = TYPE_CLAY;
// } else if (type == BlockType.Stone || type == BlockType.Stone
// || type == BlockType.StoneBrick || type == BlockType.StoneBricksSlab
// || type == BlockType.StoneBrickStairs || type == BlockType.StoneButton
// || type == BlockType.StonePressurePlate || type == BlockType.StoneSlab
// || type == BlockType.Andesite || type == BlockType.Diorite
...
// || type == BlockType.OakDoor || type == BlockType.Fence
// || type == BlockType.FenceGate || type == BlockType.OakPlanks
// || type == BlockType.OakStairs || type == BlockType.OakWoodSlab
// || type == BlockType.DoubleOakWoodSlab || type == BlockType.SpruceDoor
// || type == BlockType.SpruceFence || type == BlockType.SpruceFenceGate
// || type == BlockType.SprucePlanks || type == BlockType.SpruceStairs
// || type == BlockType.SpruceWoodSlab || type == BlockType.DoubleSpruceWoodSlab
// || type == BlockType.WoodenButton || type == BlockType.WoodenPressurePlate
// || type == BlockType.Lever || type == BlockType.Trapdoor
// || type == BlockType.Workbench || type == BlockType.Chest
// || type == BlockType.TrappedChest || type == BlockType.Torch
// || type == BlockType.Ladder || type == BlockType.StandingSign
...
// || type == BlockType.QuartzPillarHorizontal
// || type == BlockType.QuartzPillarVertical || type == BlockType.QuartzSlab
// || type == BlockType.QuartzStairs || type == BlockType.ChiseledQuartzBlock
// || type == BlockType.DoubleQuartzSlab) {
...
// || type == BlockType.DoubleSandStoneSlab
// || type == BlockType.DoubleRedSandstoneSlab) {
...
// || type == BlockType.GlowStone || type == BlockType.Gravel
// || type == BlockType.Prismarine || type == BlockType.PrismarineBricks
// || type == BlockType.SeaLantern || type == BlockType.SoulSand
// || type == BlockType.Sponge || type == BlockType.SnowBlock
...
// || type == BlockType.RedstoneComparator
// || type == BlockType.RedstoneComparatorPowered) {
...
// valueType = TYPE_RAIL;
// } else if (type == BlockType.Bed || type == BlockType.JackOLantern
// || type == BlockType.Lilypad || type == BlockType.NoteBlock
// || type == BlockType.StandingBanner || type == BlockType.TripwireHook
// || type == BlockType.WallBanner || type == BlockType.Bookshelf
// || type == BlockType.Flowerpot || type == BlockType.Skull) {
// valueType = TYPE_DECORATION;
// } else if (type == BlockType.Dispenser || type == BlockType.Dropper
// || type == BlockType.Piston || type == BlockType.PistonExtended
// || type == BlockType.PistonHead || type == BlockType.Cauldron
// || type == BlockType.Hopper || type == BlockType.StickyPiston
// || type == BlockType.TNT || type == BlockType.Anvil
// || type == BlockType.BrewingStand || type == BlockType.EnchantmentTable
// || type == BlockType.EnderChest || type == BlockType.Jukebox) {
...
// || type == BlockType.JungleFence || type == BlockType.JungleFenceGate
// || type == BlockType.Fence || type == BlockType.FenceGate
// || type == BlockType.SpruceFence || type == BlockType.SpruceFenceGate
...
// } else
// if (type == BlockType.CoalOre || type == BlockType.IronOre || type == BlockType.GoldOre
...
// || type == BlockType.OakDoor || type == BlockType.SpruceDoor
// || type == BlockType.IronDoor || type == BlockType.Furnace
// || type == BlockType.BurningFurnace || type == BlockType.Chest
...
// } else if (type == BlockType.RedstoneTorchOff || type == BlockType.RedstoneTorchOn
// || type == BlockType.Bed || type == BlockType.JackOLantern
// || type == BlockType.Lilypad || type == BlockType.NoteBlock
// || type == BlockType.StandingBanner || type == BlockType.TripwireHook
// || type == BlockType.WallBanner || type == BlockType.Dispenser
// || type == BlockType.Dropper || type == BlockType.Piston
...
// || type == BlockType.RedstoneRepeaterOff || type == BlockType.RedstoneRepeaterOn
// || type == BlockType.Bookshelf || type == BlockType.Flowerpot
// || type == BlockType.Cauldron || type == BlockType.Hopper
// || type == BlockType.StickyPiston || type == BlockType.TNT) {
// valueVariant = VARIANT_MODERATE;
// } else if (type == BlockType.DaylightSensor || type == BlockType.DaylightSensorInverted
// || type == BlockType.RedstoneComparator
// || type == BlockType.RedstoneComparatorPowered || type == BlockType.Skull
// || type == BlockType.Anvil || type == BlockType.BrewingStand
