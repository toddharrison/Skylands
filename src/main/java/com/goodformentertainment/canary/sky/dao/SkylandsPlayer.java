...
public class SkylandsPlayer extends DataAccess {
    ...
    public static SkylandsPlayer getXPlayer(final Player player)
            throws DatabaseReadException, DatabaseWriteException {
        SkylandsPlayer xPlayer = new SkylandsPlayer();
        final Map<String, Object> filters = new HashMap<String, Object>();
        filters.put(SkylandsPlayer.PLAYER_UUID, player.getUUIDString());
        Database.get().load(xPlayer, filters);

        if (xPlayer.hasData()) {
            return xPlayer;
        } else {
            xPlayer = new SkylandsPlayer();
    ...
    public SkylandsPlayer() {
        super("skylands_xplayer");
    }

    @Override
    public SkylandsPlayer getInstance() {
        return new SkylandsPlayer();
    }
    ...
    public void update() throws DatabaseWriteException {
        final Map<String, Object> filters = new HashMap<String, Object>();
        filters.put(SkylandsPlayer.PLAYER_UUID, playerUuid);
